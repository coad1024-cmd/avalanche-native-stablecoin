...
/**
 * @title ResetControllerBuggy (Reference Bug-Preserving Implementation)
 * @notice Preserves CONTRA-01 / VULN-01: Denominator Price Squaring Reset Flapping.
 */
contract ResetControllerBuggy is IResetController {
    ...
    function checkReset() public view override returns (ResetType, uint256 currentNAV_B) {
        ...
        // BUG: Compounding beta * P_0 in denominator
    ...
    function executeReset() external override returns (ResetType) {
        ...
        uint256 livePrice = getLivePrice();
        uint256 P_0 = vault.referencePrice();
        // BUG: newBeta is computed as (livePrice / P_0), then both are saved to vault
