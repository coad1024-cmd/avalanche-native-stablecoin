# Spec Miner Survey Handoff Report: Academic Literature & Whitepaper Derivation Audit
...
4. **Simulation Codebase (`simulations/cadcad_core/`):**
   ...
   * *`dynamic_resets.py`:* Implements Theorem 1 single-step crash tolerance formula returning $-60.00\%$ (no subsidy) and $-58.15\%$ (with $10\%$ subsidy at $T=100\text{ days}$; $-53.51\%$ at $T=1\text{ yr}$; SSRN reports $-52.4\%$ at $v \approx 1.2\text{ yr}$).
