# Academic Literature & Whitepaper Specification Survey: anUSD First-Principles Derivation Audit
...
### 3.1 Dual-Class Securitization Architecture & Alpha Formulation ($lpha = 0.5$ vs $lpha = 1.0$)
...
Under the $1:1$ issuance ratio ($lpha = 1.0$), depositing $M_C$ collateral at reference price $P_0$ and conversion factor $\beta_t$ mints $N$ shares of Class A and $N$ shares of Class B, where:
...
### 3.5 Downward Reset Mechanics, Conversion Factor $eta$, and Crash Bounds ($-60.0\%$ vs $-75.0\%$)
...
| 4 | $\chi$ / $lpha$ | `tranche_ratio_chi` | Tranching | $lpha = 0.5$ (Sec 2) / $1.0$ (App A) | **1.0000** | $[0.80, 1.20]$ | $[0.20, 5.00]$ | Ratio | **Major Notation Shift:** SSRN Section 2 defines $lpha = 0.5$ (capital fraction); Whitepaper defines $lpha = 1.0$ (issuance ratio). Both yield $2.0\times$ leverage. |
...
2. **cadCAD Dynamic Reset & Crash Verification:**
   ...
   print(f'Crash Bound (With Subsidy, T=100d): {bound_with_sub * 100:.2f}% (Expected: -58.15%)')
   "
   ```
